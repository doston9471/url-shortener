class ShortenedUrlsController < ApplicationController
  before_action :find_url, only: [:show, :shortened, :rename]
  ...
  def create
    ...
    if @url.new_url?
      ...
      flash[:notice] = "A short link for this URL is already in your database"
      redirect_to shortened_path(@url.find_duplicate.short_url)
    end
  end

  def rename_url
    if @url.update(short_url: params[:short_url])
      flash[:notice] = "A short link for this URL is already in your database"
      redirect_to shortened_path(@url.short_url)
    else
      flash[:error] = "Check the error below:"
